...
const EXTRA_TICKET_MANAGER_ROLE_ID = process.env.EXTRA_TICKET_MANAGER_ROLE_ID || "1458217061837705311";
...
// ✅ Emoji custom per titolo welcome / pannelli
const WELCOME_TITLE_EMOJI = "<:icona_ticket:1467182266554908953>";

// ✅ ROSTER SOLDATI
const ROSTER_SOLDATI_CHANNEL_ID = process.env.ROSTER_SOLDATI_CHANNEL_ID || "1467797145716527278";
const ROSTER_SOLDATI_ROLE_A_ID = process.env.ROSTER_SOLDATI_ROLE_A_ID || "1467583255514448043";
const ROSTER_SOLDATI_ROLE_B_ID = process.env.ROSTER_SOLDATI_ROLE_B_ID || "1458217336312959178";
...
async function tryAssignAutoJoinRole(member, origin = "unknown") {
  ...
  await debugSend(guild, `❌ [AUTO-ROLE] Fallito assegnare ruolo a ${member.user?.tag ?? member.id}.`);
}
...
// ✅ Roster state (salva messageId come il panel)
async function getRosterSoldatiMessageId(guildId, channelId) {
  const key = `roster_soldati:${guildId}:${channelId}`;
  if (redis) {
    try {
      if (redis.status !== "ready") await redis.connect().catch(() => {});
      return await redis.get(key);
...
async function setRosterSoldatiMessageId(guildId, channelId, messageId) {
  const key = `roster_soldati:${guildId}:${channelId}`;
  if (redis) {
    try {
      if (redis.status !== "ready") await redis.connect().catch(() => {});
...
// ================== ROSTER SOLDATI UI (V2) ==================
function clampMentions(mentions, max = 80) {
  const arr = Array.isArray(mentions) ? mentions : [];
  if (arr.length <= max) return { shown: arr, remaining: 0 };
  return { shown: arr.slice(0, max), remaining: arr.length - max };
}

async function buildRosterSoldatiComponents(guild) {
  // assicura members completi per avere conteggio/liste corrette
  try {
    await guild.members.fetch();
  } catch {}

  const roleA = await guild.roles.fetch(ROSTER_SOLDATI_ROLE_A_ID).catch(() => null);
  const roleB = await guild.roles.fetch(ROSTER_SOLDATI_ROLE_B_ID).catch(() => null);

  const roleAMembers = roleA ? [...roleA.members.values()] : [];
  const roleBMembers = roleB ? [...roleB.members.values()] : [];

  const sortByName = (m1, m2) => {
    const a = (m1.displayName || m1.user?.username || "").toLowerCase();
    const b = (m2.displayName || m2.user?.username || "").toLowerCase();
    return a.localeCompare(b);
  };

  roleAMembers.sort(sortByName);
  roleBMembers.sort(sortByName);

  const aMentions = roleAMembers.map((m) => `<@${m.id}>`);
  const bMentions = roleBMembers.map((m) => `<@${m.id}>`);

  const aClamped = clampMentions(aMentions, 80);
  const bClamped = clampMentions(bMentions, 80);

  const inner = [
    { type: 10, content: `# <:icona_ticket:1467182266554908953> **Fam. Gotti - Gerarchia Soldati**` },
    { type: 14, divider: true, spacing: 2 },

    // Sezione A
    { type: 10, content: `> - <@&${ROSTER_SOLDATI_ROLE_A_ID}> [${roleA ? roleA.members.size : 0}]` },
    {
      type: 10,
      content:
        aClamped.shown.length > 0
          ? `${aClamped.shown.map((x) => `- ${x}`).join("\n")}${aClamped.remaining > 0 ? `\n- ... +${aClamped.remaining} altri` : ""}`
          : "- (Nessun membro)",
    },

    // separatore come pannello ticket
    { type: 14, divider: true, spacing: 2 },

    // Sezione B
    { type: 10, content: `> - <@&${ROSTER_SOLDATI_ROLE_B_ID}> [${roleB ? roleB.members.size : 0}]` },
    {
      type: 10,
      content:
        bClamped.shown.length > 0
          ? `${bClamped.shown.map((x) => `- ${x}`).join("\n")}${bClamped.remaining > 0 ? `\n- ... +${bClamped.remaining} altri` : ""}`
          : "- (Nessun membro)",
    },
  ];
...
async function upsertRosterSoldatiMessage(channel) {
  const lock = await acquireGlobalLock(`roster_soldati:${channel.guild.id}:${channel.id}`, 12_000);
  if (!lock.ok) return null;

  try {
    // 1) prova a cancellare quello già salvato
    const existingId = await getRosterSoldatiMessageId(channel.guild.id, channel.id);
    if (existingId) {
      const existing = await channel.messages.fetch(existingId).catch(() => null);
      if (existing && existing.author?.id === client.user.id) {
    ...
    // 2) fallback: cerca nei messaggi recenti un vecchio “Gerarchia Soldati” del bot
    try {
      const recent = await channel.messages.fetch({ limit: 30 });
      const old = recent.find((m) => {
        if (m.author?.id !== client.user.id) return false;
        try {
          const raw = JSON.stringify(m.components ?? []);
          return raw.includes("Gerarchia Soldati") && raw.includes("Fam. Gotti");
        } catch {
          return false;
        }
      });
    ...
    // invio nuovo
    const components = await buildRosterSoldatiComponents(channel.guild);
    const sent = await channel.send({
      flags: MessageFlags.IsComponentsV2,
      components,
      allowedMentions: { parse: [] },
    ...
    await setRosterSoldatiMessageId(channel.guild.id, channel.id, sent.id);
    return sent;
  } finally {
    await lock.release();
  }
...
const commands = [
  ...
  new SlashCommandBuilder()
    .setName("roster_soldati")
    .setDescription("Invia/aggiorna la Gerarchia Soldati (solo admin, solo canale dedicato).")
...
function bindEventsOnce() {
  ...
  client.on("interactionCreate", async (interaction) => {
    try {
      ...
      // ================== /roster_soldati ==================
      if (interaction.isChatInputCommand() && interaction.commandName === "roster_soldati") {
        ...
        // solo in canale specifico
        if (interaction.channelId !== ROSTER_SOLDATI_CHANNEL_ID) {
          await interaction
            .reply({
              content: `❌ Questo comando si può usare solo in <#${ROSTER_SOLDATI_CHANNEL_ID}>.`,
        ...
        if (!ch || !ch.isTextBased?.()) {
          await interaction.reply({ content: "❌ Canale non valido.", flags: MessageFlags.Ephemeral }).catch(() => {});
          return;
        }

        await interaction.deferReply({ ephemeral: true }).catch(() => {});
        await upsertRosterSoldatiMessage(ch);
      ...
      if (interaction.isButton() && String(interaction.customId).startsWith("ba_choose:")) {
        ...
        const openerId = extractUserIdFromTopic(interaction.channel.topic) || interaction.user.id;
        ...
        await interaction.channel
          .send({
            content: `<@${openerId}>: **Hai Selezionato la Categoria:** \`${picked}\``,
